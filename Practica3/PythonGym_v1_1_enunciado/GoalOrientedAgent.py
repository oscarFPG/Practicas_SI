...
class GoalOrientedAgent(BaseAgent):
    ...
    def InitAgent(self,perception,map):
        ...
        # - Crear el problema asociado al entorno de BatttelCity - creamos el problema con BCProblem
        xsize, ysize = 15, 15
        self.problem = BCProblem(initialNode, goal1CommanCenter, xsize, ysize) 
        # - Inicializamos el mapa - inicializamos el mapa problem.InitMap
        self.problem.InitMap(map)
        # - inicializamos A* con el problema creado
        self.aStar =AStar(self.problem)
        # - creamos un plan inicial - a partir de la percepcion y el mapa
        self.plan = self._CreatePlan(perception, map)
        print("Plan inical generado")
        GoalOrientedAgent.ShowPlan(self.plan)
        # - Generar objetivos adicionales: vida y jugador
